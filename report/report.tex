...
\lstset{
  ...
  morekeywords={pub, public, private, class, var, mut, self, fn, impl, struct, match, where, move, let, trait, static, ref}
}
...
%\begin{lstlisting}[float=*,caption={Factory container definition},label={lst:factory1}]
...
\end{minipage}
\lstinputlisting[float=*,caption={Dependency resolution and construction},label={lst:scope2}]{listings/scope2.txt}
...
\begin{minipage}{\linewidth}
\lstinputlisting[caption={The backing store can be reused for any type},label={lst:trait_usage}]{listings/trait_usage.txt}
\end{minipage}
...
\begin{minipage}{\linewidth}
\lstinputlisting[caption={The backing store can be reused for any type},label={lst:trait_macro_usage}]{listings/trait_macro_usage.txt}
\end{minipage}
